<template>
  <div>
        ...
        <el-dropdown :width="400" trigger="hover">
          ...
          <template #dropdown>
            <el-dropdown-menu class="custom-dropdown-menu">
              <el-dropdown-item
                v-if="currentUser.role === 'ADMIN' || currentUser.idUser === userData.idUser"
                class="dropdownitem"
                @click="isDialogProfile = true"
              >
    ...
    <!-- 4 Dialog for Edit Education isDialogQualification-->
    <el-dialog
...
<script lang="js">
  //import { useAppOptionStore } from '.cree./../../store/app/options'
  //import { AUTH_CURRENT_USER } from '@/core/constants'
    ...
    import router from '@/router' //Different User Profile


    export default {
      name: 'UserProfile',
    setup() {
        const idUser = router.currentRoute.value.params.id //Different User Profile
        ...
        const initializeUserData = async () => {
          try {
            state.userData = await userProfile.actGetUserById(idUser)//Different User Profile
            ...
            state.selectProfile = {
              ...
              qualifications: state.userData.qualifications ? state.userData.qualifications.map(qualification => ({
                id: qualification.id,
        ...
        const getVcardApi = async () => {
          try {
            console.log('Step 1: state.userData.idUser', state.userData.idUser)
