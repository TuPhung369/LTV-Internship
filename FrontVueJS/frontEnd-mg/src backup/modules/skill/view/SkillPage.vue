<template>
  <div>
    ...
    <!-- Edit Dialog -->
    <el-dialog v-model="editDialogVisible" title="Edit Skill" width="500px">
    ...
    <!-- Delete Dialog -->
    <el-dialog v-model="deleteDialogVisible" title="Delete Skill" width="500px">
    ...
    <!-- Create Dialog -->
    <el-dialog v-model="createDialogVisible" title="Create Skill" width="500px">
...
<script>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const getSkillService = new skillService()
      const fetchDataPositions = async () => {
        try {
          const response = await getSkillService.getSkill()
      ...
      const handleEdit = async () => {
        try {
          await getSkillService.updateSkill(editForm.value.id, editForm.value)
      ...
      const handleCreate = async () => {
        try {
          await getSkillService.createSkill(createForm.value)
      ...
      const handleDelete = async () => {
        try {
          await getSkillService.deleteSkill(selectedRow.value.id)
          await fetchDataPositions()
