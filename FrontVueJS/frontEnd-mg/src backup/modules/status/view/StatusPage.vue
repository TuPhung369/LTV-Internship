<template>
  <div>
    ...
    <el-table :data="tableData" stripe style="width: 100%">
      <el-table-column prop="title" label="Status" label-class-name="custom-label-padding">
    ...
    <!-- Edit Dialog -->
    <el-dialog v-model="editDialogVisible" title="Edit Status" width="500px">
    ...
    <!-- Delete Dialog -->
    <el-dialog v-model="deleteDialogVisible" title="Delete Status" width="500px">
      <span>Are you sure you want to delete this Status?</span>
      <template #footer>
    ...
    <!-- Create Dialog -->
    <el-dialog v-model="createDialogVisible" title="Create Status" width="500px">
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const getStatusService = new StatusService()
      const fetchDataStatus = async () => {
        try {
          const response = await getStatusService.getStatus()
      ...
      const handleEdit = async () => {
        try {
          await getStatusService.updateStatus(editForm.value.id, editForm.value)
      ...
      const handleCreate = async () => {
        try {
          await getStatusService.createStatus(createForm.value)
          createDialogVisible.value = false
          await fetchDataStatus()
          ElMessage.success('Status created successfully')
        } catch (error) {
          console.error(error)
          ElMessage.error('Failed to create Status')
        }
      }
      const handleDelete = async () => {
        try {
          await getStatusService.deleteStatus(selectedRow.value.id)
          await fetchDataStatus()
