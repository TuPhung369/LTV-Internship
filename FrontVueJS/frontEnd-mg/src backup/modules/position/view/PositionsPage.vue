<template>
  <div>
    ...
    <!-- Edit Dialog -->
    <el-dialog v-model="editDialogVisible" title="Edit Position" width="500px">
    ...
    <!-- Delete Dialog -->
    <el-dialog v-model="deleteDialogVisible" title="Delete Position" width="500px">
    ...
    <!-- Create Dialog -->
    <el-dialog v-model="createDialogVisible" title="Create Position" width="500px">
...
<script>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const getPositionService = new PositionsService()
      const fetchDataPositions = async () => {
        try {
          const response = await getPositionService.getPosition()
      ...
      const handleEdit = async () => {
        try {
          await getPositionService.updatePosition(editForm.value.id, editForm.value)
          await fetchDataPositions()
          editDialogVisible.value = false
          ElMessage.success('Position updated successfully')
        } catch (error) {
          console.error(error)
          ElMessage.error('Failed to update position')
        }
      }
      const handleCreate = async () => {
        try {
          await getPositionService.createPosition(createForm.value)
          createDialogVisible.value = false
          await fetchDataPositions()
          ElMessage.success('Position created successfully')
        } catch (error) {
          console.error(error)
          ElMessage.error('Failed to create position')
        }
      }
      const handleDelete = async () => {
        try {
          await getPositionService.deletePosition(selectedRow.value.id)
          await fetchDataPositions()
